...
TEST_AURORACOIN_BINARY="src/test/test_auroracoin"
...
if [[ ! -e ${TEST_AURORACOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_AURORACOIN_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_AURORACOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
    ...
    rm "${TEST_OUTPUT_TEMPFILE}"
    if [[ $(get_file_suffix_count gcda) == 0 ]]; then
        echo "Error: Running the test suite did not create any *.gcda files. The gcda files are generated when the instrumented test programs are executed. R
        exit 1
    fi
