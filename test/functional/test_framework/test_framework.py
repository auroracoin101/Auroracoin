...
class AuroracoinTestFramework(metaclass=AuroracoinTestMetaClass):
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None):
        ...
        for i in range(num_nodes):
            self.nodes.append(TestNode(i, get_datadir_path(self.options.tmpdir, i), rpchost=rpchost, timewait=self.rpc_timewait, auroracoind=binary[i],auroracoin_cli=self.options.auroracoincli, mocktime=self.mocktime, coverage_dir=self.options.coveragedir, extra_conf=extra_confs[i], extra_args=extra_args[i], use_cli=self.options.usecli))
