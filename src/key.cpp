...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    if (pubkey.IsCompressed() != fCompressed)
        return false;
    unsigned char rnd[8];
    std::string str = "Auroracoin key verification\n";
...
bool CKey::LoadLegacy(CPrivKey &privkey, CPubKey &vchPubKey, bool fSkipCheck=false) {
    CECKey key;
    if (!key.SetPrivKey(privkey.data(), privkey.size(), fSkipCheck))
        return false;

    key.GetSecretBytes((unsigned char*)begin());
    fCompressed = vchPubKey.IsCompressed();
    fValid = true;
