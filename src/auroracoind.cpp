...
static bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/auroracoin.conf are parsed in qt/auroracoin.cpp's main()
    SetupServerArgs();
    std::string error;
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  auroracoind [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                tfm::format(std::cerr, "Error: Command line contains unexpected token '%s', see auroracoind -h for a list of options.\n", argv[i]);
                return false;
            }
        }

        // -server defaults to true for auroracoind but not for the GUI so do this here
...
#endif
            tfm::format(std::cout, "Auroracoin server starting\n");
