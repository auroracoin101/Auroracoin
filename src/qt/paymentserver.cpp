...
const int AURORACOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString AURORACOIN_IPC_PREFIX("auroracoin:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/auroracoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/auroracoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/auroracoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the auroracoin: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(AURORACOIN_IPC_PREFIX, Qt::CaseInsensitive)) // auroracoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseAuroracoinURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(AURORACOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(AURORACOIN_IPC_CONNECT_TIMEOUT);
...
//
// OSX-specific way of handling auroracoin: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("auroracoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'auroracoin://' is not a valid URI. Use 'auroracoin:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(AURORACOIN_IPC_PREFIX, Qt::CaseInsensitive)) // auroracoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseAuroracoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Auroracoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom auroracoin addresses are not supported
        ...
        // Auroracoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(AuroracoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
