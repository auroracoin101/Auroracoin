...
{
    window = new AuroracoinGUI(m_node, platformStyle, networkStyle, nullptr);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", AuroracoinGUI::tr("A fatal error occurred. Auroracoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", AuroracoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6. Determine availability of data directory and parse auroracoin.conf
