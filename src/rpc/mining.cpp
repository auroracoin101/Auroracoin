...
static UniValue generateBlocks(const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries, int algo)
{
    int nHeightEnd = 0;
    int nHeight = 0;
    ...
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(coinbase_script, algo));
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
        ...
        throw std::runtime_error(
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated auroracoin to."},
    ...
    return generateBlocks(coinbase_script, nGenerate, nMaxTries, algo);
}
