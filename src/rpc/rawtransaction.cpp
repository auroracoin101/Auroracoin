...
static void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    // Call into TxToUniv() in auroracoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in auroracoin-common, so we query them here and push the
    // data into the returned UniValue.
    //TxToUniv(tx, uint256(), entry, true, RPCSerializationFlags());  Bitpay code change, commented out for now
...
UniValue decodepsbt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "          \"hex\" : \"hex\",            (string) The hex\n"
            "          \"type\" : \"pubkeyhash\",    (string) The type, eg 'pubkeyhash'\n"
            "          \"address\" : \"address\"     (string) Auroracoin address if there is one\n"
            "        }\n"
            "      },\n"
