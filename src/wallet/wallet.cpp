...
void CWallet::ReacceptWalletTransactions(interfaces::Chain::Lock& locked_chain)
{
    ...
    for (const std::pair<const int64_t, CWalletTx*>& item : mapSorted) {
        ...
        // wtx.AcceptToMemoryPool(*locked_chain, state); Bitcoin: this method has the argument locked_chain that interferes here.
    }
}
...
bool CWalletTx::RelayWalletTransaction(interfaces::Chain::Lock& locked_chain)
{
    ...
    if (!IsCoinBase() && !isAbandoned() && GetDepthInMainChain(locked_chain) == 0)
    {
        ...
        LogPrintf("Transaction in memory pool: %u, Accepted To MemoryPool: %u", InMempool(), AcceptToMemoryPool(locked_chain, state));
