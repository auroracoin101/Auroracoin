...
bool Consensus::CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, bool strict)
{
            ...
            if (coin.IsCoinBase()) {
                if (coin.nHeight < 225001) {
                    ...
                    if (nSpendHeight - coin.nHeight < COINBASE_MATURITY_2) {
                        int whitelisted_blocks[] = {337402, 339159, 339353, 339825, 340651, 341886, 462188,
                                                   737576, 743345, 1114698, 1263754, 1412186, 2213689, 2219851};
                        if (std::find(std::begin(whitelisted_blocks), std::end(whitelisted_blocks), nSpendHeight) != std::end(whitelisted_blocks)) {
                            // These blocks got accepted because the wallets after the multi-algo switch initially 
                            // didn't validate this. We whitelist these blocks here to reinstate this rule.
                            ...
                            if (strict) {
                                ...
                                LogPrintf("tried to spend coinbase at depth %d.\n", nSpendHeight - coin.nHeight);
                            }
                        }
                    }
                }
