...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", AURORACOIN_CONF_FILENAME), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", AURORACOIN_PID_FILENAME), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    ...
    // Bitpay code
    gArgs.AddArg("-addressindex", strprintf("Maintain a full address index, used to query for the balance, txids and unspent outputs for addresses. (default: %u)", DEFAULT_ADDRESSINDEX), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-timestampindex", strprintf("Maintain a timestamp index for block hashes, used to query blocks hashes by a range of timestamps. (default: %u)", DEFAULT_TIMESTAMPINDEX), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-spentindex", strprintf("Maintain a full spent index, used to query the spending txid and input index for an outpoint. (default: %u)", DEFAULT_SPENTINDEX), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-algo=<algo>", "Mining algorithm: sha256d, scrypt, groestl, skein, qubit", ArgsManager::ALLOW_ANY, true, OptionsCategory::BLOCK_CREATION);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/aurarad/auroracoin>";
    const std::string URL_WEBSITE = "<https://auroracoin.is>";
...
/** Sanity checks
 *  Ensure that Auroracoin is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    for (const auto& arg : gArgs.GetUnsuitableSectionOnlyArgs()) {
        InitWarning(strprintf(_("Config setting for %s only applied on %s network when in [%s] section.").translated, arg, network, network));
    }
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", AURORACOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if auroracoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if auroracoin is started while in a temporary directory.\n",
