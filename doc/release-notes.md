Auroracoin version 0.18.0 is now available from:
...
`contrib/init/auroracoind.service`, it has been changed to use
`/var/lib/auroracoind` as the data directory instead of
`~auroracoin/.auroracoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/auroracoind` will
exist has enough space (check using `df -h /var/lib/auroracoind`), and
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:12341:12341` (this is an extra `:12341` over the
...
The systemd init file (`contrib/init/auroracoind.service`) has been changed
to use `/var/lib/auroracoind` as the data directory instead of
`~auroracoin/.auroracoin`. This change makes Auroracoin more consistent with
...
`datadir` option in `/etc/auroracoin/auroracoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/auroracoin/auroracoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/aurarad/Auroracoin/blob/master/doc/JSON-RPC-interface.md)
...
- Further information is added to the [JSON-RPC
  documentation](https://github.com/aurarad/Auroracoin/blob/master/doc/JSON-RPC-interface.md)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/aurarad/Auroracoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Auroracoin knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Auroracoin, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/auraras/Auroracoin/blob/master/doc/descriptors.md).
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart auroracoind with the `-deprecatedrpc=generate`
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Auroracoin
...
- When fetching a transaction announced by multiple peers, previous
  versions of Auroracoin would sequentially attempt to download the
...
- A sub-project of Bitcoin Core now provides Hardware Wallet Interaction
  ...
  hardware key management devices with Auroracoin.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  ...
  gathered by Auroracoin is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Auroracoin a
