...
Auroracoin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
To profile a running auroracoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep auroracoind` -- sleep 60
```
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Auroracoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- src/univalue
  - Upstream at https://github.com/bitcoin-core/univalue ; actively maintained by Bitcoin Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with Auroracoin's `select()` loop, because
