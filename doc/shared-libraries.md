...
The interface is defined in the C header `auroracoinconsensus.h` located in `src/script/auroracoinconsensus.h`.

#### Version

`auroracoinconsensus_version` returns an `unsigned int` with the API version *(currently `1`)*.
